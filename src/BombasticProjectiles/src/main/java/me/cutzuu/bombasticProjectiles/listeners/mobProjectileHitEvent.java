    ...
    private void whichImpactType()
    {
        if (launchCheck.launchGlobal.mobShotStarted)
        {
            if (!main.Global.configToggleFreeMode)
            {
                ...
                {
                    if (main.Global.configToggleVerbose) getServer().broadcastMessage("§eProjectile Notice §7- §cMobBreakBlocks is toggled off.");
                ...
                //----------------------------------------------------------------------------
                if (main.Global.configToggleVerbose) getServer().broadcastMessage(mEventInfo.mString + mEventInfo.mImpactValue);
